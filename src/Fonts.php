...
class Fonts
{
    ...
    public static function addTTFfont($fontfile, $fonttype = '', $enc = '', $flags = 32, $outpath = '', $platid = 3, $encid = 1, $addcbbox = false, $link = false) {
        if (!StaticUtils::file_exists($fontfile)) {
            // Could not find file
            return false;
        }
        // font metrics
        ...
        if (@StaticUtils::file_exists($outpath . $font_name . '.php')) {
        ...
        if (empty($fonttype)) {
            if (StaticUtils::_getULONG($font, 0) == 0x10000) {
        ...
        if ($fmetric['type'] == 'Type1') {
            ...
            $fmetric['file'] .= '.z';
            $fp = StaticUtils::fopenLocal($outpath . $fmetric['file'], 'wb');
            ...
            if (StaticUtils::_getULONG($font, $offset) != 0x10000) {
            ...
            $numTables = StaticUtils::_getUSHORT($font, $offset);
            $offset += 2;
            ...
            for ($i = 0; $i < $numTables; ++$i) {
                ...
                $offset += 4;
                $table[$tag] = array();
                $table[$tag]['checkSum'] = StaticUtils::_getULONG($font, $offset);
                $offset += 4;
                $table[$tag]['offset'] = StaticUtils::_getULONG($font, $offset);
                $offset += 4;
                $table[$tag]['length'] = StaticUtils::_getULONG($font, $offset);
                $offset += 4;
            }
            // check magicNumber
            $offset = $table['head']['offset'] + 12;
            if (StaticUtils::_getULONG($font, $offset) != 0x5F0F3CF5) {
            ...
            $offset += 2; // skip flags
            // get FUnits
            $fmetric['unitsPerEm'] = StaticUtils::_getUSHORT($font, $offset);
            $offset += 2;
            // units ratio constant
            $urk = (1000 / $fmetric['unitsPerEm']);
            $offset += 16; // skip created, modified
            $xMin = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $yMin = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $xMax = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $yMax = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $fmetric['bbox'] = '' . $xMin . ' ' . $yMin . ' ' . $xMax . ' ' . $yMax . '';
            $macStyle = StaticUtils::_getUSHORT($font, $offset);
            $offset += 2;
            // PDF font flags
            $fmetric['Flags'] = $flags;
            if (($macStyle & 2) == 2) {
            ...
            $short_offset = (StaticUtils::_getSHORT($font, $offset) == 0);
            $offset += 2;
            ...
            if ($short_offset) {
                ...
                for ($i = 0; $i < $tot_num_glyphs; ++$i) {
                    $indexToLoc[$i] = StaticUtils::_getUSHORT($font, $offset) * 2;
                ...
                for ($i = 0; $i < $tot_num_glyphs; ++$i) {
                    $indexToLoc[$i] = StaticUtils::_getULONG($font, $offset);
                    if (isset($indexToLoc[($i - 1)]) && ($indexToLoc[$i] == $indexToLoc[($i - 1)])) {
                        // the last glyph didn't have an outline
                        unset($indexToLoc[($i - 1)]);
                    }
            ...
            $numEncodingTables = StaticUtils::_getUSHORT($font, $offset);
            $offset += 2;
            $encodingTables = array();
            for ($i = 0; $i < $numEncodingTables; ++$i) {
                $encodingTables[$i]['platformID'] = StaticUtils::_getUSHORT($font, $offset);
                $offset += 2;
                $encodingTables[$i]['encodingID'] = StaticUtils::_getUSHORT($font, $offset);
                $offset += 2;
                $encodingTables[$i]['offset'] = StaticUtils::_getULONG($font, $offset);
                $offset += 4;
            }
            // ---------- get os/2 metrics ----------
            ...
            // xAvgCharWidth
            $fmetric['AvgWidth'] = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // usWeightClass
            $usWeightClass = round(StaticUtils::_getUFWORD($font, $offset) * $urk);
            ...
            $fsType = StaticUtils::_getSHORT($font, $offset);
            $offset += 2;
            if ($fsType == 2) {
            ...
            $numNameRecords = StaticUtils::_getUSHORT($font, $offset);
            $offset += 2;
            // Offset to start of string storage (from start of table).
            $stringStorageOffset = StaticUtils::_getUSHORT($font, $offset);
            $offset += 2;
            for ($i = 0; $i < $numNameRecords; ++$i) {
                $offset += 6; // skip Platform ID, Platform-specific encoding ID, Language ID.
                // Name ID.
                $nameID = StaticUtils::_getUSHORT($font, $offset);
                $offset += 2;
                if ($nameID == 6) {
                    // String length (in bytes).
                    $stringLength = StaticUtils::_getUSHORT($font, $offset);
                    $offset += 2;
                    // String offset from start of storage area (in bytes).
                    $stringOffset = StaticUtils::_getUSHORT($font, $offset);
                    $offset += 2;
            ...
            $fmetric['italicAngle'] = StaticUtils::_getFIXED($font, $offset);
            $offset += 4;
            $fmetric['underlinePosition'] = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $fmetric['underlineThickness'] = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $isFixedPitch = (StaticUtils::_getULONG($font, $offset) == 0) ? false : true;
            $offset += 2;
            if ($isFixedPitch) {
                $fmetric['Flags'] |= 1;
            }
            ...
            // Ascender
            $fmetric['Ascent'] = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // Descender
            $fmetric['Descent'] = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // LineGap
            $fmetric['Leading'] = round(StaticUtils::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // advanceWidthMax
            $fmetric['MaxWidth'] = round(StaticUtils::_getUFWORD($font, $offset) * $urk);
            $offset += 2;
            $offset += 22; // skip some values
            // get the number of hMetric entries in hmtx table
            $numberOfHMetrics = StaticUtils::_getUSHORT($font, $offset);
            // ---------- get maxp data ----------
            $offset = $table['maxp']['offset'];
            $offset += 4; // skip Table version number
            // get the the number of glyphs in the font.
            $numGlyphs = StaticUtils::_getUSHORT($font, $offset);
            // ---------- get CIDToGIDMap ----------
            $ctg = array();
            $c = 0;
                    ...
                    switch ($format) {
                        ...
                        case 2: { // Format 2: High-byte mapping through table
                            ...
                            for ($i = 0; $i < 256; ++$i) {
                                ...
                                $subHeaderKeys[$i] = (StaticUtils::_getUSHORT($font, $offset) / 8);
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $numSubHeaders; ++$k) {
                                $subHeaders[$k]['firstCode'] = StaticUtils::_getUSHORT($font, $offset);
                                $offset += 2;
                                $subHeaders[$k]['entryCount'] = StaticUtils::_getUSHORT($font, $offset);
                                $offset += 2;
                                $subHeaders[$k]['idDelta'] = StaticUtils::_getUSHORT($font, $offset);
                                $offset += 2;
                                $subHeaders[$k]['idRangeOffset'] = StaticUtils::_getUSHORT($font, $offset);
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $numGlyphIndexArray; ++$k) {
                                $glyphIndexArray[$k] = StaticUtils::_getUSHORT($font, $offset);
                                $offset += 2;
                            }
                            for ($i = 0; $i < 256; ++$i) {
                                $k = $subHeaderKeys[$i];
                        ...
                        case 6: { // Format 6: Trimmed table mapping
                            $offset += 4; // skip length and version/language
                            $firstCode = StaticUtils::_getUSHORT($font, $offset);
                            $offset += 2;
                            $entryCount = StaticUtils::_getUSHORT($font, $offset);
                            $offset += 2;
                            for ($k = 0; $k < $entryCount; ++$k) {
                                $c = ($k + $firstCode);
                                $g = StaticUtils::_getUSHORT($font, $offset);
                                $offset += 2;
                                $ctg[$c] = $g;
                            }
                            break;
                        ...
                        case 8: { // Format 8: Mixed 16-bit and 32-bit coverage
                            ...
                            for ($k = 0; $k < 8192; ++$k) {
                                $is32[$k] = StaticUtils::_getBYTE($font, $offset);
                                ++$offset;
                            }
                            $nGroups = StaticUtils::_getULONG($font, $offset);
                            $offset += 4;
                            for ($i = 0; $i < $nGroups; ++$i) {
                                $startCharCode = StaticUtils::_getULONG($font, $offset);
                                $offset += 4;
                                $endCharCode = StaticUtils::_getULONG($font, $offset);
                                $offset += 4;
                                $startGlyphID = StaticUtils::_getULONG($font, $offset);
                                $offset += 4;
                        ...
                        case 10: { // Format 10: Trimmed array
                            ...
                            $startCharCode = StaticUtils::_getULONG($font, $offset);
                            $offset += 4;
                            $numChars = StaticUtils::_getULONG($font, $offset);
                            $offset += 4;
                        ...
                        case 12: { // Format 12: Segmented coverage
                            ...
                            for ($k = 0; $k < $nGroups; ++$k) {
                                ...
                                $endCharCode = StaticUtils::_getULONG($font, $offset);
                                $offset += 4;
                                $startGlyphCode = StaticUtils::_getULONG($font, $offset);
                                $offset += 4;
            ...
            $yMin = StaticUtils::_getFWORD($font, $offset);
            $offset += 4;
            $yMax = StaticUtils::_getFWORD($font, $offset);
            $offset += 2;
            ...
            for ($i = 0; $i < $numberOfHMetrics; ++$i) {
                $cw[$i] = round(StaticUtils::_getUFWORD($font, $offset) * $urk);
                $offset += 4; // skip lsb
            }
            ...
            for ($cid = 0; $cid <= 65535; ++$cid) {
                if (isset($ctg[$cid])) {
                    ...
                    if ($addcbbox and isset($indexToLoc[$ctg[$cid]])) {
                        ...
                        $xMin = round(StaticUtils::_getFWORD($font, $offset + 2) * $urk);
                        $yMin = round(StaticUtils::_getFWORD($font, $offset + 4) * $urk);
                        $xMax = round(StaticUtils::_getFWORD($font, $offset + 6) * $urk);
                        $yMax = round(StaticUtils::_getFWORD($font, $offset + 8) * $urk);
        ...
        if ($fmetric['type'] == 'Type1') {
            ...
            if ($fmetric['type'] == 'cidfont0') {
                ...
                $fp = StaticUtils::fopenLocal($outpath . $fmetric['ctg'], 'wb');
        ...
        $pfile .= '// --- EOF ---' . "\n";
        // store file
        $fp = StaticUtils::fopenLocal($outpath . $font_name . '.php', 'w');
        fwrite($fp, $pfile);
        fclose($fp);
        ...
        while (!empty($new_sga)) {
            ...
            foreach ($sga as $key => $val) {
                if (isset($indexToLoc[$key])) {
                    ...
                    $numberOfContours = StaticUtils::_getSHORT($font, $offset);
                    $offset += 2;
                    if ($numberOfContours < 0) { // composite glyph
                        $offset += 8; // skip xMin, yMin, xMax, yMax
                        do {
                            $flags = StaticUtils::_getUSHORT($font, $offset);
                            $offset += 2;
                            $glyphIndex = StaticUtils::_getUSHORT($font, $offset);
                            $offset += 2;
    ...
    public static function getFontFullPath($file, $fontdir = false) {
        ...
        if (($fontdir !== false) and @StaticUtils::file_exists($fontdir . $file)) {
            $fontfile = $fontdir . $file;
        } elseif (@StaticUtils::file_exists(self::_getfontpath() . $file)) {
            $fontfile = self::_getfontpath() . $file;
        } elseif (@StaticUtils::file_exists($file)) {
            $fontfile = $file;
        }
        return $fontfile;
    }
...
// ====================================================================================================================
     ...
     * @param bool $isunicode True for Unicode mode, false otherwise.
     ...
     * @param bool $isunicode True when the documetn is in Unicode mode, false otherwise.
     ...
     * @param bool $isunicode True if the document is in Unicode mode, false otherwise.
     * @param array $currentfont Reference to current font array.
     * @return string
     * @author Nicola Asuni
     * @since 2.1.000 (2008-01-08)
    ...
    public static function utf8Bidi($ta, $str, $forcertl, $isunicode, &$currentfont) {
        ...
        // max level
        $maxlevel = 0;
        if (StaticUtils::empty_string($str)) {
